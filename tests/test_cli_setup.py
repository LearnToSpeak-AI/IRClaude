...
def test_setup_writes_toml_and_calls_ergo_fetch(monkeypatch, tmp_path: Path):
    ...
    monkeypatch.setattr("irclaude.cli.download_ergo", fake_download, raising=False)
    ...
    monkeypatch.setattr(
        "irclaude.cli.detect_weechat_plugin_dir",
        lambda: tmp_path / "weechat" / "python" / "autoload",
        raising=False,
    )
    monkeypatch.setattr("irclaude.cli.weechat_running", lambda: True, raising=False)

    result = runner.invoke(app, ["setup"], input="y\n")
    assert result.exit_code == 0
    cfg = home / ".config" / "irclaude" / "config.toml"
    assert cfg.exists()
...
def test_setup_weechat_symlink_creates_plugin_link(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "irclaude.cli.detect_weechat_plugin_dir", lambda: plugin_dir, raising=False
    )
    repo_plugin = tmp_path / "repo" / "weechat_plugin" / "irclaude.py"
    repo_plugin.parent.mkdir(parents=True)
    repo_plugin.write_text("# plugin", encoding="utf-8")
    monkeypatch.setattr("irclaude.cli.repo_plugin_path", lambda: repo_plugin, raising=False)
    monkeypatch.setattr("irclaude.cli.weechat_running", lambda: True, raising=False)

    result = runner.invoke(app, ["setup-weechat"])
    assert result.exit_code == 0
    linked = plugin_dir / "irclaude.py"
    assert linked.exists() or linked.is_symlink()


def test_setup_weechat_prints_manual_fallback_when_dir_missing(monkeypatch):
    monkeypatch.setattr("irclaude.cli.detect_weechat_plugin_dir", lambda: None, raising=False)
    result = runner.invoke(app, ["setup-weechat"])
    assert result.exit_code == 0
