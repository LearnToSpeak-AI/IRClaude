...
from irclaude.config import Settings, load_settings


def test_default_settings_use_xdg_paths(monkeypatch, tmp_path):
    monkeypatch.delenv("IRCLAUDE_APPS_ROOT", raising=False)
    monkeypatch.delenv("IRCLAUDE_DATA_DIR", raising=False)
    monkeypatch.delenv("IRCLAUDE_CONFIG_FILE", raising=False)
    ...
    assert s.data_dir == tmp_path / ".local" / "share" / "irclaude"
    assert s.config_file == tmp_path / ".config" / "irclaude" / "config.toml"
...
def test_xdg_overrides_apply(monkeypatch, tmp_path):
    ...
    s = Settings()
    assert s.data_dir == tmp_path / "xdg-data" / "irclaude"
    assert s.config_file == tmp_path / "xdg-conf" / "irclaude" / "config.toml"


def test_env_overrides_xdg(monkeypatch, tmp_path):
    monkeypatch.setenv("IRCLAUDE_APPS_ROOT", str(tmp_path / "apps"))
    monkeypatch.setenv("IRCLAUDE_DATA_DIR", str(tmp_path / "data"))
    monkeypatch.setenv("IRCLAUDE_PORT", "9999")
    s = Settings()
...
def test_db_path_and_mcp_paths_are_under_data_dir(monkeypatch, tmp_path):
    monkeypatch.setenv("IRCLAUDE_DATA_DIR", str(tmp_path / "d"))
    s = Settings()
...
def test_load_settings_merges_toml_file(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("IRCLAUDE_CONFIG_FILE", str(cfg))
    s = load_settings()
...
def test_env_wins_over_toml(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("IRCLAUDE_CONFIG_FILE", str(cfg))
    monkeypatch.setenv("IRCLAUDE_PORT", "8200")
    s = load_settings()
    assert s.port == 8200
