...
def test_status_shows_running_when_pid_alive(monkeypatch, tmp_path):
    ...
    (data / "run" / "irclaude.pid").write_text(str(os.getpid()), encoding="utf-8")
...
def test_stop_signals_pid_and_removes_file(monkeypatch, tmp_path):
    data = tmp_path / "d"; (data / "run").mkdir(parents=True)
    pid_file = data / "run" / "irclaude.pid"
    sent: list[tuple[int, int]] = []

    def fake_kill(pid, sig):
        sent.append((pid, sig))

    monkeypatch.setattr("irclaude.cli.os.kill", fake_kill, raising=False)
...
def test_start_writes_pid_file(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("IRCLAUDE_DATA_DIR", str(data))
    monkeypatch.setattr(
        "irclaude.cli._launch_bridge_blocking", lambda settings: None, raising=False
    )
